...
def index_view(request):
    return render(request,'astroapp/index.html')
...
def login_view(request):
    if request.method=='POST':
        ...
        try:
            ...
            return render(request,'astroapp/login.html',{'error_msg':error_msg})
    error_msg=request.GET.get('error_msg',None)    
    return render(request,'astroapp/login.html',{'error_msg':error_msg})

def signup_view(request):
    return render(request,'astroapp/signup.html')
...
def email_varification_view(request):
    if request.method=='POST':
        ...
        if UserInfo.objects.filter(email=email):
             ...
             return render(request,'astroapp/signup.html',error_msg)
        ...
        try:
            ...
            return render(request,'astroapp/verify_otp.html',{'success_msg':success_msg,'otp':otp,'email':email,'password':set_password,'name':name})
...
def otp_verify_view(request):
      if request.method=='POST':
          ...
          if otp==sent_code:
              ...
              user.save()
              return render(request,'astroapp/login.html',success_msg)
          else:
               msg={'verify_msg':'wrong otp'}
               return render(request,'astroapp/signup.html')  
      else:
          return render(request,'astroapp/verify_otp.html')   
...
def forecast_view(request,index):
        if 'user_id' in request.session:
            ...
            try:
                ...
                flag_response=None   
            Base_URL= f"http://api.weatherapi.com/v1//forecast.json?key={key}&q={location}&days=12" 
            try:
                    ...
                    day=weekdays[index]
                    return render(request,'astroapp/forecast.html',{'weather_data':weather_data,'weekdays':weekdays,'location':location,'day':day})
            except:
                  return render(request,'astroapp/forecast.html')
        else:    
...
def weather_view(request):
    if 'user_id' in request.session:
        ...
        try:
            ...
            return render(request,'astroapp/weather.html',{'weather_data':data,'weekday_name':weekdays[2:]})
        except:
